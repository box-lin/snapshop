			...
			for(int y = 1; y< getWidth()-1;y++){
				...
				for (int k=0; k<3;k++) {
					for (int j=0; j<3;j++) {
						...
						Pixel points = data1 [x-1+k][y-1+j];   
						
						redsubtotal = redsubtotal + points.red*weights[k][j];
						greensubtotal = greensubtotal + points.green*weights[k][j];
						bluesubtotal = bluesubtotal + points.blue*weights[k][j];
					}
				}
				
				int averageR = redsubtotal / total;
				int averageG = greensubtotal / total;
				int averageB = bluesubtotal / total;
				...
				data2[x][y]= new Pixel(averageR,averageG,averageB);
